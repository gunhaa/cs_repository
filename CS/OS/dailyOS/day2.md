...
- qemu가 플로피디스크 삽입시의 동작을 따라하고, 해당 프로그램은 동작에 맞는 프로그램 이미지를 넣어서 동작을 재현한다
- 플로피디스크가 첫번째 섹터(512 바이트)를 로드 하고 실행시키는 과정을 재현한다
- 요약하자면
    1. 부트로더 작성 (플로피디스크의 첫 번째 섹터에 들어갈 코드)
    2. 부트로더가 포함된 플로피디스크 이미지 생성 (LoopyOS.img)
    3. QEMU에서 가상 플로피디스크로 사용 (실제 디스크처럼 동작)