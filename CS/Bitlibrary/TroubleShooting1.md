...
또한 컨트롤러에서도 코드가 직관적이지 않아 직관적인 코드가 될 수 있도록 할 필요성을 느껴 리팩토링을 진행하였다.

## 문제 해결 과정
1. 루프 최적화 및 Dto 반환

뷰 템플릿에서 불필요한 3중 루프를 제거하고, 필요한 데이터를 한 번의 로직으로 추출하여 DTO로 반환하는 방식으로 개선하였다. 이를 통해 처리 속도가 향상되었으며, 결과 값을 다른 페이지에서도 활용할 수 있는 구조를 만들었다.
