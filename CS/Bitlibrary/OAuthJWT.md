...
- 사실상 모든 보안 방식은 약점이 있고 각자의 장점이 있으므로, XSS, CSRF를 당할 경우 하나는 보존 할 수 있도록 두가지 방법으로 저장한다.
- access토큰은 현재 프로젝트에선 뷰 템플릿 방식을 쓰므로 react처럼 메모리에 넣는 방식을 채택할수는 없고, localstorage를 이용한다.
