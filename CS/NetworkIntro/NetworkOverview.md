...
| 계층 | 이름               | 주요 목적                                                   |
| -- | ---------------- | ------------------------------------------------------------------- |
| 7  | **Application**  | 사용자와 직접 상호작용하며 응용 서비스(HTTP, FTP 등)를 제공한다. 데이터 표현보다는 서비스 제공에 초점을 둔다. |
| 6  | **Presentation** | 데이터의 형식, 인코딩, 암호화, 압축 등을 처리하여 응용 계층 간 호환성을 보장한다.                    |
| 5  | **Session**      | 통신 세션의 생성·유지·종료를 관리하며, 연결의 동기화와 복구를 담당한다.                           |
| 4  | **Transport**    | 종단 간 신뢰성 있는 데이터 전송(TCP) 또는 비연결성 전송(UDP)을 제공한다.                      |
| 3  | **Network**      | 라우팅과 IP 주소를 이용해 패킷을 목적지까지 전달한다. 경로 선택이 핵심이다.                        |
| 2  | **Data Link**    | 프레임 단위로 오류 제어와 흐름 제어를 수행하며, MAC 주소를 이용해 전송한다.                       |
| 1  | **Physical**     | 전기·광신호로 데이터를 실제 매체(케이블, 무선 등)에 전송한다. 하드웨어적 전송 계층이다.                 |

- 현대 Network는 Application layer가 Application + Presentation + Session의 모든 기능을 제공하는 것으로 이야기하며, 5 layer로 많이 사용한다(응용 서비스 제공, 데이터 인코딩·압축·암호화, 세션 관리까지 모두 담당)
- TCP Connection도 Session이라고 표현하지만, session layer는 논리적인 연결을 칭하며 tcp의 session은 실제 session의 구현체로 사용된다