...
```plain text
수많은 블로그를 읽으며 혼란스러웠다. 
누가 옳은지, 어떤 말이 맞는지 알기 어려웠다. 
그래서 스스로 올바른 자료를 찾아 정리하기 시작했다.

모르는 개념을 검색하다 보면 같은 키워드를 끝없이 반복하게 된다. 
그 시간을 줄이고, 내가 직접 쌓은 지식의 흔적을 남기고 싶었다. 

블로그보다는 단순하고 자유롭게 다루기 위해 GitHub에서 Markdown으로 학습 내용을 정리하고 있다.
```
