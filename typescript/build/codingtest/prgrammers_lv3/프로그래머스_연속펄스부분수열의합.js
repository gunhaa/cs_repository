...
function solution14(sequence) {
    let searchMax = [];
    for (let i = 0; i < sequence.length; i++) {
        for (let j = 0; j < sequence.length; j++) {
            let tempArr = sequence.slice(i, j + 1);
    ...
    const maxNumber = searchMax.reduce((a, b) => {
        return a > b ? a : b;
    });
    return maxNumber;
}
function solution13(sequence) {
