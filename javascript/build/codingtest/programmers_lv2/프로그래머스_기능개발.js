...
*/
function solution91(progresses, speeds) {
    /* pseudocode
...
function solution92(progresses, speeds) {
    let answer = [];
    let progress = progresses;
    while (progress.length !== 0) {
        progress = progress.map((item, index) => item + speeds[index]);
        if (progress[0] >= 100) {
            console.log(progress);
            let completeItems = 0;
            completeItems++;
...
function solution93(progresses, speeds) {
    let answer = [];
    let progress = progresses;
    while (progress.length !== 0) {
        progress = progress.map((item, index) => item + speeds[index]);
