...
function solution_makeSquare(arr : number[][]) {
    ...
    if(row>col){
        ...
        const len = col;
        const addItem : number[] = Array.from<number>({length : len}).fill(0);
...
// 해당 방식으로 solve할시 정답은 맞지만, 현재 2번 테스트케이스에서 0,0,0 으로 만들어낸 배열이 같은 곳을 참조한다. 
// Array.from을 사용할때 주의해야 한다.